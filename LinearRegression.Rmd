...
| If I took my Y data and averaged them, what would be my notation?| I would get $\bar{Y}$.|
...
Here is a link that Brother Saunders used for <a href="https://phet.colorado.edu/sims/html/least-squares-regression/latest/least-squares-regression_en.html">Linear Regression</a>.  

Explain the meaning of this equation.  
$$
  \underbrace{\hat{Y}_i}_\text{ }= \overbrace{b_0}^\text{est. y-int} + \overbrace{b_1}^\text{est. slope} \underbrace{X_i}_\text{Specific point}
$$
$\hat{Y}_i$ = a point on the regression line that gives the average y-value for a specific x-value.  

|Skills Quiz: Question | Answer|
|----------|----------|
| What does linear mean?| |
| What are we telling the error?| That they need to be normal and constant and that the error terms are centered around 0.|
| What is a coefficent?| |
| What is a regression line?| |
| What is does linear mean?| |
| Why is it important to include the simple linear regression math model?| It allows everyone to understand what notations (symbols) you are using and what they mean. It helps you to understand and connect with the class information.|
|What are the required elements to a well done simple linear regression analysis?| |
|What is the difference between a graphic and a diagnostic plot?| |
|How do you use "math chunks" to write out the simple linear regression model and its corresponding set of hypotheses?| |
|Why is it important to include the simple linear regression model along with your hypotheses about that model in your analysis?| |
|How do you interpret the values for the slope and intercept of a simple linear regression?| |

|What do the five assumptions of the simple linear regression model assume?| |
|-----|-----|
|Linear relation| |
|Normal errors| |
|Constant variance| |
...
|Large sample (n > 30)| |
|Normal Y values| |
|Distributions similarly shaped| |
...
<!--  Notes from class
      ...
      mtext(side=3, "Residuals vs. Order")
